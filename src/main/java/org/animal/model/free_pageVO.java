...
public class free_pageVO {
	private int startPage; // 시작 번호
	private int endPage; // 끝번호
	private boolean prev; // 이전버튼
	private boolean next; // 다음버튼
	private int free_total; // board테이블의 전체건수를 저장
	// fcriteriaVO 포함
	private free_CriteriaVO fcri;

	// 생성자(매개변수가 2개인 생성자) 선언
	public free_pageVO(free_CriteriaVO fcri, int free_total) {
		this.fcri = fcri;
		...
		// 123*1.0 = 123.0 /10
		// 12.3(올림) > 13
		int realEnd = (int) (Math.ceil((free_total * 1.0) / fcri.getAmount()));
	...
	public free_CriteriaVO getFcri() {
		return fcri;
	}

	public void setFcri(free_CriteriaVO fcri) {
		this.fcri = fcri;
	}
	...
	public String toString() {
		...
		return "pageVO [startPage=" + startPage + ", endPage=" + endPage + ", prev=" + prev + ", next=" + next
				+ ", fcri=" + fcri + ", free_total=" + free_total + "]";
	}
}
